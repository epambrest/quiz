...
namespace CodeTester
{
    public class Compiler
    {
        ...
        {
            var compilationType = new CSharpCompilationOptions(OutputKind.ConsoleApplication);
            var syntaxTrees = new[] { CSharpSyntaxTree.ParseText(codeToCompile) };
            var compilation = CSharpCompilation.Create(GenerateFilePath(), syntaxTrees, references, compilationType);
            EmitResult result = compilation.Emit(fileFullPath);
            return GenerateCompilResult(result);
        }
        private string GenerateFilePath()
        {
            string filePath = Path.GetRandomFileName().Split(".").First() + ".exe";
            fileFullPath = Path.Combine(DirectoryPathToCompile, filePath);
